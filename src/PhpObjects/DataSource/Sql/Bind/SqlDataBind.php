...
class SqlDataBind
{
    ...
    private function _makeSqlValueString( $value )
    {
        if ( $value === null ) {
            return 'NULL';
        }
    ...
    private function _bindSqlData( $sql, array $binding, array &$tableDescription = [] )
    {
        foreach ( $binding as $bindKey => $bindValue ) {
            ...
            if ( self::$_UsePregMatchForBindSqlData ) // Die sicherere, aber aufwendigere variante !!!
            {
                ...
                $sql = str_replace($bindKey, self::_makeSqlValueString($bindValue) . $appendChar, $sql);
            } else {
                // Ersetzt nur das erste vorkommen !!!
                $sql = substr_replace($sql, self::_makeSqlValueString($bindValue), strpos($sql, $bindKey), strlen($bindKey));
            }
        }
